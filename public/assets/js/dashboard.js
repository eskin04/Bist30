...
(
  ...
  $(function() {
    ...
    if ($("#transaction-history").length) {
      var bakiye = $("#bakiye").text()
      bakiye = bakiye.replace('₺', '')
      bakiye = bakiye.replaceAll('.', '')
      bakiye = parseInt(bakiye)
      var hisse = $("#toplam").text()
      hisse = hisse.replace('₺', '')
      hisse = hisse.replaceAll('.', '')
      hisse = parseInt(hisse)
      let toplam = bakiye + hisse
      // toplamın başına ₺ koyar
      toplam = toplam.toLocaleString('tr-TR', { maximumFractionDigits: 0, style: 'currency', currency: 'TRY' })
      // toplama m ekler
      // kaç tane nokta olduğunu sayar
      let nokta = toplam.split('.').length - 1
      if(nokta >= 2){
        toplam = toplam.slice(0, -5)
        toplam = toplam + 'M'
      }
      else{
        toplam = toplam.slice(0, -2)
        toplam = toplam + 'B'
      }
      

      var areaData = {
        labels: ["Hisse", "Kullanılabilir"],
        datasets: [{
            data: [hisse, bakiye],
            backgroundColor: [
              "#ffab00", "#00d25b"
